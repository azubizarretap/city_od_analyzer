...
-- Generamos vista con ubicacion de transportes asociados a eventos
CREATE OR REPLACE VIEW COA.V_Paradas_Eventos AS

SELECT DISTINCT P.ID_PARADA,P.Latitud,P.Longitud,T.Equipamiento As Nombre_Linea ,TT.Nombre AS Tipo_Transporte
FROM	
	COA.F_PARADA P,
	COA.D_TRANSPORTE T,
	COA.A_TIPO_TRANSPORTE TT,
	COA.D_PARADAS_EVENTO PE
WHERE
	P.ID_PARADA=PE.ID_PARADA
	AND P.ID_TRANSPORTE=T.ID_TRANSPORTE
	AND T.ID_TIPO_TRANSPORTE=TT.ID_TIPO_TRANSPORTE;

--Generamos vista con la informacion de los eventos y su ubicacion
CREATE OR REPLACE VIEW COA.V_UBICACION_EVENTOS AS
SELECT DISTINCT UE.ID_UBICACION_EVENTO,UE.Latitud,UE.Longitud,E.Nombre As Nombre_Evento ,E.Tipo_Evento AS Tipo_Evento, NE.nombre AS Nivel_Evento
FROM	
...
WHERE
	UE.ID_UBICACION_EVENTO=UES.ID_UBICACION_EVENTO
	AND UES.ID_EVENTO=E.ID_EVENTO
	AND E.ID_NIVEL_EVENTO=NE.ID_NIVEL_EVENTO;

--Generamos vista donde se relacionan los transportes con los eventos
CREATE OR REPLACE VIEW V_TRANSPORTE_EVENTOS AS

SELECT DISTINCT E.ID_EVENTO,P.Latitud AS Latitud_Parada,P.Longitud As Longitud_Parada,T.Equipamiento As Nombre_Linea ,TT.Nombre AS Tipo_Transporte,
		UE.Latitud As Latitud_Evento,UE.Longitud As Longitud_Evento,E.Nombre As Nombre_Evento ,E.Tipo_Evento AS Tipo_Evento, NE.nombre AS Nivel_Evento
FROM	
	COA.F_PARADA P,
	COA.D_TRANSPORTE T,
	COA.A_TIPO_TRANSPORTE TT,
...
WHERE
	...
	AND P.ID_TRANSPORTE=T.ID_TRANSPORTE
	AND T.ID_TIPO_TRANSPORTE=TT.ID_TIPO_TRANSPORTE
	AND UE.ID_UBICACION_EVENTO=UES.ID_UBICACION_EVENTO
	AND UES.ID_EVENTO=E.ID_EVENTO
	AND E.ID_NIVEL_EVENTO=NE.ID_NIVEL_EVENTO
	AND PE.ID_UBICACION_EVENTO=UE.ID_UBICACION_EVENTO;

CREATE OR REPLACE VIEW V_TEST_TRANSPORTE_EVENTOS AS
...
UNION
SELECT	DISTINCT T.Equipamiento As Nombre ,F.Latitud As Latitud,F.Longitud As Longitud, 'Transporte' As Tipo
FROM
	COA.F_PARADA F,
	COA.D_TRANSPORTE T,
	...
	COA.D_PARADAS_EVENTO PE
WHERE
	PE.ID_PARADA = F.ID_PARADA
	AND F.ID_TRANSPORTE = T.ID_TRANSPORTE
